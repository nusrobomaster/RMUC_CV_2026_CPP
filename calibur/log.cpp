...
void Logger::setFormatter(const std::string& val) {
    calibur::LogFormatter::ptr new_val(new calibur::LogFormatter(val));
    if(new_val->isError()) {
...
void LogAppender::setFormatter(const std::string& val) {
    if(val != "") {
        calibur::LogFormatter::ptr new_val(new LogFormatter(val));
        if(new_val->isError()) {
...
calibur::ConfigVar<std::set<LogDefine>>::ptr g_log_defines = 
    calibur::Config::Lookup("logs", std::set<LogDefine>{}, "logs config");

struct LogIniter {
    LogIniter() {
        g_log_defines->addListener(0xF1E231, [](const std::set<LogDefine>& old_value, const std::set<LogDefine>& new_value){
            // 新增
            CALIBUR_LOG_INFO(CALIBUR_LOG_ROOT()) << "On logger ref has been changed";
            for(auto& i : new_value) {
                auto it = old_value.find(i);
                calibur::Logger::ptr logger;
                if(it == old_value.end()) {
                    //新增logger
                    logger = CALIBUR_LOG_NAME(i.name);
                } else {
                    if(!(i == *it)) {
                        //修改logger
                        logger = CALIBUR_LOG_NAME(i.name);
            ...
            for(auto& i : old_value) {
                ...
                if(it == new_value.end()) {
                    //删除logger
                    auto logger = CALIBUR_LOG_NAME(i.name);
...
// reset(CALIBUR_LOG_NAME.get())	Bypasses refcounting	Unsafe (double-free)	Never use this!
// CALIBUR_LOG_NAME()	            Proper refcount++	    Safe	                Normal global logger access
