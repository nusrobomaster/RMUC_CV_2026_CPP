...
void DisplayWorker::operator()() {
    ...
    while (!stop_flag_.load(std::memory_order_relaxed)) {
        ...
        draw_target_dot_and_fps(img,pred,cam_ptr->width,cam_ptr->height,fps);
...
//  DRAW TARGET DOT (predicted hit point using yaw/pitch)
// ============================================================================
void DisplayWorker::draw_target_dot_and_fps(cv::Mat &img,
                                    const PredictionOut *pred,
                                    int width, int height,double fps)
{
    char buf[64];
    std::snprintf(buf, sizeof(buf), "FPS = %.1f", fps);
    cv::putText(img, buf, cv::Point(20,40),
            cv::FONT_HERSHEY_SIMPLEX, 0.7,
            cv::Scalar(0, 255, 0), 2);
