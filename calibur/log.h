#ifndef __CALIBUR_LOG_H__
#define __CALIBUR_LOG_H__
...
/**
 * @def CALIBUR_LOG_LEVEL(logger, level)
 * @brief Base logging macro for specified level
 * 
 * Usage: CALIBUR_LOG_LEVEL(logger_ptr, level) << "message";
 * 
 * Only evaluates if logger's level threshold permits
 */
#define CALIBUR_LOG_LEVEL(logger, level) \
    if(logger->getLevel() <= level) \
        calibur::LogEventWrap(calibur::LogEvent::ptr(new calibur::LogEvent(logger, level, \
          __FILE__, __LINE__, 0, calibur::GetThreadId(), \
            calibur::GetFiberId(), time(0)))).getSS()

// Convenience macros for standard levels
#define CALIBUR_LOG_DEBUG(logger) CALIBUR_LOG_LEVEL(logger, calibur::LogLevel::DEBUG)
#define CALIBUR_LOG_INFO(logger) CALIBUR_LOG_LEVEL(logger, calibur::LogLevel::INFO)
#define CALIBUR_LOG_WARN(logger) CALIBUR_LOG_LEVEL(logger, calibur::LogLevel::WARN)
#define CALIBUR_LOG_ERROR(logger) CALIBUR_LOG_LEVEL(logger, calibur::LogLevel::ERROR)
#define CALIBUR_LOG_FATAL(logger) CALIBUR_LOG_LEVEL(logger, calibur::LogLevel::FATAL)

#define CALIBUR_LOG_FMT_LEVEL(logger, level, fmt, ...) \
	if(logger->getLevel() <= level)	\
		calibur::LogEventWrap(calibur::LogEvent::ptr(new calibur::LogEvent(logger, level, \
			__FILE__, __LINE__, 0, calibur::GetThreadId(), \
		calibur::GetFiberId(), time(0)))).getEvent()->format(fmt, __VA_ARGS__)

/**
 * @def CALIBUR_LOG_FMT_LEVEL(logger, level, fmt, ...)
 * @brief Formatted logging macro for specified level
 * 
 * Usage: CALIBUR_LOG_FMT_LEVEL(logger, level, "format %s", arg)
 */
#define CALIBUR_LOG_FMT_DEBUG(logger, fmt, ...)  CALIBUR_LOG_FMT_LEVEL(logger, calibur::LogLevel::DEBUG, fmt, __VA_ARGS__)
#define CALIBUR_LOG_FMT_INFO(logger, fmt, ...)   CALIBUR_LOG_FMT_LEVEL(logger, calibur::LogLevel::INFO, fmt, __VA_ARGS__)
#define CALIBUR_LOG_FMT_WARN(logger, fmt, ...)   CALIBUR_LOG_FMT_LEVEL(logger, calibur::LogLevel::WARN, fmt, __VA_ARGS__)
#define CALIBUR_LOG_FMT_ERROR(logger, fmt, ...)  CALIBUR_LOG_FMT_LEVEL(logger, calibur::LogLevel::ERROR, fmt, __VA_ARGS__)
#define CALIBUR_LOG_FMT_FATAL(logger, fmt, ...)  CALIBUR_LOG_FMT_LEVEL(logger, calibur::LogLevel::FATAL, fmt, __VA_ARGS__)

// Root logger access  
#define CALIBUR_LOG_ROOT() calibur::LoggerMgr::GetInstance()->getRoot()
#define CALIBUR_LOG_NAME(name) calibur::LoggerMgr::GetInstance()->getLogger(name);
...
public:
    ...
    std::string toYamlString();
private:
    friend class calibur::Singleton<LoggerManager>;
...
typedef calibur::Singleton<LoggerManager> LoggerMgr;
