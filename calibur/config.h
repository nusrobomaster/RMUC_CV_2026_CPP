#ifndef __CALIBUR_CONFIG_H__
#define __CALIBUR_CONFIG_H__
...
public:
    ...
    std::string toString() override {
        try {
            ...
            CALIBUR_LOG_ERROR(CALIBUR_LOG_ROOT()) 
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,
        ...
        if(tmp) {
            CALIBUR_LOG_INFO(CALIBUR_LOG_ROOT()) << "Lookup name:" << name << " exists";
            return tmp;
        } 
        ...
        if(name.find_first_not_of("abcdefghikjlmnopqrstuvwxyz._0123456789")
                != std::string::npos) {
            CALIBUR_LOG_ERROR(CALIBUR_LOG_ROOT()) << "Lookup name invalid:" << name;
